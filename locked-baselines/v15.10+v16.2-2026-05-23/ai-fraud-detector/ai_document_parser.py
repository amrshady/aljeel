...
def _validate_row(row: dict, row_idx: int) -> tuple[bool, str]:
    ...
    # ticket_no must be present and non-empty
    ticket = row.get("ticket_no")
    if not ticket or str(ticket).strip() == "":
        return False, f"Row {row_idx}: ticket_no is empty"
    return True, ""
...
_GEMINI_MODEL_CASCADE = ["gemini-2.5-flash-preview-05-20", "gemini-2.5-pro", "gemini-pro-latest"]
