...
def resolve_line(
    ...
    if emp is None:
        ...
        if account == "60307021":
            ...
            # v15.6: override derive_sponsor_segments hardcoded 10100 with itinerary-based location
            _sp_loc, _, _sp_loc_flag = _resolve_location_v15_7(description)
            location = _sp_loc
            if _sp_loc_flag:
                flags.append(_sp_loc_flag)
            cost_center = sponsor_segs["cost_center"]
            div = sponsor_segs["div"]
            agency = sponsor_segs["agency"]
            solution = sponsor_segs["solution"]
        else:
            # Non-sponsor, not in master — use v15.6 deterministic location
            _loc_code, _, _loc_flag = _resolve_location_v15_7(description)
            location = _loc_code
            _loc_flag_nm = _loc_flag  # applied after flags.append(EMPLOYEE_NOT_IN_MASTER)
            cost_center = "999999"
            div = "000"
            solution = "00000"
            agency = "00000"
        ...
        # v15.6: Manpower location field is IRRELEVANT for GL location.
        # Location is determined purely by trip itinerary (IATA-based deterministic rule).
        _loc_code, _loc_layer, _loc_flag = _resolve_location_v15_7(description)
        location = _loc_code
        if _loc_flag:
