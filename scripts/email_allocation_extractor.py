...
_GEMINI_MODEL_CASCADE = ["gemini-2.5-pro", "gemini-2.5-flash"]
...
def _call_gemini(prompt_text: str, api_key: str) -> Optional[dict]:
    """Try Gemini model cascade and return parsed JSON result."""
    for model in _GEMINI_MODEL_CASCADE:
        try:
            ...
            import urllib.error
            # 403 / 404 / 429 / 5xx: try next model in cascade
            is_transient = (
                isinstance(exc, urllib.error.HTTPError) and exc.code in (403, 404, 429, 500, 502, 503, 504)
...
def _call_llm(prompt_text: str) -> tuple[Optional[dict], str]:
    ...
    if gemini_key:
        try:
            ...
            print(f"[email_allocation_extractor] Gemini failed: {exc}. No fallback configured.")
    return None, ""
