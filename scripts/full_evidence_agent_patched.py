...
GEMINI_PRICING = {
    # Gemini 3 Pro and 2.5 Pro share pricing; Flash is much cheaper.
    "gemini-pro-latest": {"in": 1.25/1e6, "out": 10.00/1e6},  # resolves to Gemini 3 Pro when live
...
**TYPE B — Personal contribution / annual ticket** (employee paying for personal travel):
  ...
  - For the AP row: account = 60301003 (still standard travel) UNLESS the OPEX form or email body contains explicit Oracle Fusion GL codes showing a different account (e.g., 60308009 for Training Expenses). If form codes are present and show a different account, use those form codes — they override the TYPE B default.
...
**TYPE D — Training Expenses (60308009)**:
  - Evidence or OPEX form explicitly refers to a training course, training programme, or educational attendance
  - OR Oracle Fusion GL codes on the form show 60308009
  - account = 60308009
  - emp_no = traveler's emp_no from the form
  - cost_center / div / solution / agency = traveler's HOME values from Manpower master
...
## Account codes — use ONLY these (official AlJeel Oracle GL index):
   - 60301003 = Travel Tickets Expense — S&M teams (IVD Solutions, Capital Equipment, Contribution, Dental & Medical Solutions, Technical Services)
   - 60301004 = Travel Cost Expense G&A — G&A employees (DIV 888)
   - 60307021 = Sponsoring Expenses — external party (doctor/guest) funded by AlJeel
   - 60308009 = Training Expenses — when OPEX form, email, or GL codes on the form show 60308009, or evidence explicitly indicates a training course/programme
   - 60308007 = Recruitment Fees — only when evidence explicitly relates to recruitment
   - 21070229 = Accrued Employee Annual Tickets — only when row is explicitly a personal deduction/contribution share
   - 21070227 = Accrued Project and Warranty Expenses — only when evidence explicitly relates to project/warranty
   - DO NOT use 11034013 (personal travel) — Amr has killed it.
   - IMPORTANT: If Oracle Fusion GL codes appear on the OPEX form or approval email showing one of the above accounts, trust the form codes over the default travel account (60301003).
...
def main():
    ...
    cost_usd = total_in * pricing["in"] + total_out * pricing["out"]
