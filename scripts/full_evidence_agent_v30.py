...
def find_ticket_folder(ticket_no: str, passenger: str, notes: str, all_folders: list[Path]) -> Path | None:
    ...
    # 2.5) Strict ticket content search across ALL PDFs (v31 Fix for Cross-Contamination)
    if ticket_no:
        import pypdf
        for f in all_folders:
            if not f.is_dir():
                ...
                for child in iter_evidence_files(f):
                    if child.name.upper().endswith(".PDF"):
                        try:
                            ...
                            # Scan up to first 2 pages for ticket number
...
def call_gemini(prompt: str, model: str, retries: int = 3) -> dict:
    ...
    for attempt in range(retries):
        try:
            ...
            parsed = {}
            if text:
                # raw_decode accepts the first complete object and ignores
                # Gemini's trailing chatter; repair handles truncated brackets.
                decoder = json.JSONDecoder()
                try:
                    parsed, _ = decoder.raw_decode(text)
                except json.JSONDecodeError:
                    parsed = json.loads(_close_unbalanced_json(text))
            ...
            try:
                ...
                pass
            last_err = f"HTTPError: HTTP Error {e.code}: {e.reason}"
            if e.code == 503 or "UNAVAILABLE" in body:
                # overloaded model — short backoff won't clear it; let the
                # caller's model cascade fall back instead of hammering it
                print(f"[gemini/{model}] HTTP 503 Service Unavailable — skipping remaining retries", file=sys.stderr)
                unavailable = True
                break
            print(f"[gemini/{model}] {last_err} {body}".rstrip(), file=sys.stderr)
            ...
            time.sleep(2 ** attempt)
    return {"json": {}, "in_tokens": 0, "out_tokens": 0, "model": model,
            "error": last_err or "unknown", "unavailable": unavailable}
...
def call_gemini_with_cascade(prompt: str) -> dict:
    ...
    for i, model in enumerate(GEMINI_MODEL_CASCADE):
        ...
        if res["json"] and not res["error"]:
            return res
        if res.get("unavailable") and i < len(GEMINI_MODEL_CASCADE) - 1:
            print(f"[gemini] {model} returned 503 — waiting 15s before trying next model...", file=sys.stderr)
            time.sleep(15)
    return res
...
* **Allocation segments (CC, DIV, Agency, Solution):** 
  ...
  3. If the form/event segment is missing, fall back to the requesting employee's home segment and explain that fallback in reasoning.
