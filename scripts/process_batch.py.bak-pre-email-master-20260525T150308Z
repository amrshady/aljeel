...
# ---------------------------------------------------------------------------
# v15.10: Voucher-format employee number loader
# For voucher rows (ticket_no = 26-XXX), the employee number is in col 2
# (Ref. No.) of the invoice Details sheet -- NOT in the Oracle template.
# ---------------------------------------------------------------------------
def _load_voucher_empno_map(batch_dir):
    """Build voucher_no -> emp_no dict from the invoice Details xlsx.

    Reads the "Details" sheet (header=0, flat table).
    Rule: ticket_no (col 3, 0-based) matches 26-NNN AND ref_no (col 2) is a
    7-digit number -> treat ref_no as the employee number.
    Handles both float (1001288.0) and string ("1001288.") ref_no formats.
    """
    import re as _re

    result = {}
    candidates = sorted(Path(batch_dir).glob("*.xlsx"))
    invoice_xlsx = None
    for c in candidates:
        if _re.match(r"J\d+-\d+\.xlsx", c.name, _re.IGNORECASE):
            invoice_xlsx = c
            break
    if not invoice_xlsx:
        return result
    ...
    try:
        df_det = pd.read_excel(str(invoice_xlsx), sheet_name="Details", header=0)
    except Exception as e:
        try:
            # Fallback: try sheet index 1 if named sheet not found
            df_det = pd.read_excel(str(invoice_xlsx), sheet_name=1, header=0)
        except Exception:
            print(f"[v15.10] Warning: could not read Details sheet from {invoice_xlsx.name}: {e}", flush=True)
            return result

    # col 2 = Ref. No., col 3 = Ticket No.
    voucher_re = _re.compile(r"^26-\d{3,}$")
    try:
        for _, row in df_det.iterrows():
            try:
                ref_raw = row.iloc[2]
                tkt_raw = str(row.iloc[3]).strip() if row.iloc[3] is not None else ""
                if not tkt_raw or not voucher_re.match(tkt_raw):
                    continue
                # Handle float (1001288.0) or string ("1001288" / "1001288.")
                if ref_raw is None or str(ref_raw).strip() in ("", "nan", "None"):
                    continue
                try:
                    emp_no_f = float(str(ref_raw).rstrip(".").strip())
                    emp_no = int(emp_no_f)
                except (ValueError, TypeError):
                    continue
                if len(str(emp_no)) != 7:
                    continue
                result[tkt_raw] = emp_no
                print(f"[v15.10] voucher_empno_map: {tkt_raw} -> {emp_no}", flush=True)
            except Exception:
                continue
    except Exception as e:
        print(f"[v15.10] Warning: voucher empno map iteration error: {e}", flush=True)
...
def process_batch(
    ...
    for i in range(header_row_idx + 1, len(df)):
        ...
        if hasattr(v2_result, "segment_overrides") and v2_result.segment_overrides:
            ...
            # v15.6: location from itinerary (Manpower location irrelevant)
