...
FINAL_NUMERIC_SEGMENT_WIDTHS = {
    "Cost Center": 6,
    "DIV": 3,
    "Solution": 5,
    "Agency": 5,
}
FINAL_NUMERIC_SEGMENT_INDEXES = {
    "Cost Center": 3,
    "DIV": 4,
    "Solution": 5,
    "Agency": 6,
...
def normalize_final_segment_cells(out_xlsx: Path, header_row: int) -> int:
    ...
    This is a write-time display normalizer only. It does not resolve accounts or
    alter Project/Intercompany/Future values; spacer/review rows without a valid
    10-part Distribution Combination are left untouched.
    """
    lookup = get_lookup()
    wb = openpyxl.load_workbook(out_xlsx)
    ws = wb.active
    ...
    combo_col = next(
        (ci for hdr, ci in headers.items() if "distribution combination" in hdr.lower()),
        None,
    )
    if not combo_col:
        wb.close()
    ...
    wb.close()
    print(
        f"[v30-segment-normalize] normalized blank/padded CC/DIV/Solution/Agency on {normalized} row(s)",
        flush=True,
    )
    return normalized
