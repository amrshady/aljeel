...
const EXCLUDED_DIRS = new Set(['output', 'raw', 'ai-poc-cache', 'audit']);
...
function isEvidenceFile(fileName) {
  const lowerExt = extname(fileName).toLowerCase();
  return lowerExt === '.pdf' || lowerExt === '.msg';
}

async function collectEvidenceFiles(batchDir) {
  const evidence = [];
  ...
  async function walk(currentDir) {
    ...
    for (const entry of entries) {
      ...
      const relativePath = normalizeRelativePath(relative(batchDir, fullPath));
      ...
      if (!entry.isFile() || !isEvidenceFile(entry.name) || segments.length < 2) {
        continue;
      }
...
function printReport(results, totals) {
  ...
  console.log('');
  console.log(`Jawal evidence seed ${dryRun ? 'dry run' : 'run'} summary`);
  console.table(rows);
...
async function main() {
  ...
  try {
    for (const invoiceNumber of batches) {
      const batchDir = join(batchRoot, `jawal-${invoiceNumber}`);

      try {
        const evidenceFiles = await collectEvidenceFiles(batchDir);
